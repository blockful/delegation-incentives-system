...
// The site-wide generic card (Figma node 5993:5627): "built by blockful", the
// program headline, a CTA pill, and the ENS-mark watermark strip along the
// bottom. No per-user data. The blockful logo and the watermark are inlined as
// data URLs (NOT fetched) so the card renders even on access-protected previews
// — a same-origin fetch there gets a 302 to the auth wall and the asset drops.
...
/* ─── Inlined assets ───
   Base64 data URLs for the generic card. Inlined (not fetched same-origin) so
   the card renders even behind Vercel Deployment Protection, where a
   same-origin fetch gets 302-redirected to the auth wall.
   - blockful logo: apps/frontend/public/blockful-white.png (Figma 5997:5729)
   - ENS watermark: Figma node 5993:5628 (PNG; its background is already
     #0082bb, so it blends seamlessly into the card field) */
const WM_W = 1120
const WM_H = 208
