...
const TierRow = styled.div<{ $isCurrent: boolean }>`
  ...
  padding: ${tokens.font.size.base} ${tokens.spacing.lg};
  border-radius: ${tokens.radius.md};
  background: ${({ $isCurrent }) =>
    $isCurrent ? tokens.color.lightGreen : tokens.color.surfaceAlt};
...
const BarSegment = styled.div<{ $filled: boolean }>`
  ...
  height: 3px;
  border-radius: 1.5px;
  background: ${({ $filled }) => ($filled ? tokens.color.darkBlue : tokens.color.border)};
      ...
      <Table data-testid="tier-table">
        {tiers.map((tier) => (
          <TierRow key={tier.index} $isCurrent={tier.isCurrent}>
            ...
            <TierRight>
              <ApyText>
                {tier.estimatedApyPct ? `${tier.estimatedApyPct}%` : '—'}
              </ApyText>
