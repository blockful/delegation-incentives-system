...
export async function handlePlanRedeemed(
  event: {
    args: { id: bigint; amountRedeemed: bigint; planRemainder: bigint };
    block: { number: bigint; timestamp: bigint };
    transaction: { hash: string };
    log: { logIndex: number };
  },
...
export async function handleHedgeyTransfer(
  event: {
...
export function registerHedgeyVestingHandlers() {
  ponder.on(
    ...
    async ({
      event,
      context,
    }: IndexingFunctionArgs<"HedgeyVesting:PlanCreated">) => {
      await handlePlanCreated(event as any, context);
    },
  );
  ponder.on(
    ...
    async ({
      event,
      context,
    }: IndexingFunctionArgs<"HedgeyVesting:PlanRedeemed">) => {
      await handlePlanRedeemed(event as any, context);
    },
  );
  ponder.on(
    ...
    async ({
      event,
      context,
    }: IndexingFunctionArgs<"HedgeyVesting:Transfer">) => {
      await handleHedgeyTransfer(event as any, context);
    },
  );
}
