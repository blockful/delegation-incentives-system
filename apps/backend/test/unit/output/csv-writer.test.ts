  ...
  return {
    ...
    rewards: [
      ...
      },
      {
        ...
        delegateReward: wei(75000000000000000000n),
...
describe("distributionToCsv", () => {
  ...
  it("serializes Wei values as decimal strings", () => {
    ...
    expect(firstReward[1]).toBe("100000000000000000000"); // delegate_reward
    expect(firstReward[2]).toBe("50000000000000000000"); // delegator_reward
    expect(firstReward[3]).toBe("150000000000000000000"); // combined_reward
  });

  it("assigns correct role for delegate+delegator", () => {
    const result = makeResult();
    const csv = distributionToCsv(result);
    const lines = csv.trim().split("\n");

    // First reward: has both delegate and delegator rewards
    const fields = lines[1].split(",");
    expect(fields[4]).toBe("both");
  });

  it("assigns correct role for delegator-only", () => {
    const result = makeResult();
    const csv = distributionToCsv(result);
    const lines = csv.trim().split("\n");

    // Second reward: only delegator reward
    const fields = lines[2].split(",");
    expect(fields[4]).toBe("delegator");
  });

  it("assigns correct role for delegate-only", () => {
    const result = makeResult();
    const csv = distributionToCsv(result);
    const lines = csv.trim().split("\n");

    // Third reward: only delegate reward
    const fields = lines[3].split(",");
    expect(fields[4]).toBe("delegate");
  });
  ...
  it("includes lottery winners not in rewards list", () => {
    ...
    expect(lastLine[1]).toBe("0"); // delegate_reward
    expect(lastLine[2]).toBe("0"); // delegator_reward
